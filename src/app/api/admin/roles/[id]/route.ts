...
import { updateAdminRole, type RoleInput } from "@/services/admin/roles";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    return jsonOk({ role: await updateAdminRole(db, session.actor, id, body) }, 200, { "Cache-Control": "no-store" });
