...
import { assignUserRole } from "@/services/admin/users";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    const body = await readJsonBody<{ roleId: unknown }>(request);
    if (!body) return jsonError(400, "bad_request", "Geçersiz istek.");
    const { id } = await context.params;
    return jsonOk({ user: await assignUserRole(db, session.actor, id, body.roleId) }, 200, { "Cache-Control": "no-store" });
