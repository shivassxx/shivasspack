...
import { createCategory, listAdminCategories, type CategoryInput } from "@/services/admin/catalog";
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    return jsonOk({ categories: await listAdminCategories(db, session.actor) }, 200, { "Cache-Control": "no-store" });
  } catch (error) {
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return jsonOk({ category: await createCategory(db, session.actor, body) }, 201, { "Cache-Control": "no-store" });
  } catch (error) {
    return adminErrorResponse(error);
  }
}
