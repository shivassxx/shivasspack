...
import { transitionSubmission } from "@/services/submissions";
...
export async function POST(
  ...
  try {
    ...
    if (!body || (body.action !== "submit" && body.action !== "withdraw")) {
      return jsonError(400, "bad_request", "Geçersiz istek.");
    }
    return jsonOk(
      { submission: await transitionSubmission(db, session.actor, id, body.action) },
      200,
