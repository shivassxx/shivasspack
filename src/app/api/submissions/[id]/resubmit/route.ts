...
import { reviseAndResubmit, type SubmissionInput } from "@/services/submissions";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    return jsonOk({ submission: await reviseAndResubmit(db, session.actor, id, body) }, 200, { "Cache-Control": "no-store" });
  } catch (error) {
