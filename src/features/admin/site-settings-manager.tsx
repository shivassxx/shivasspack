...
async function saveSetting(input: { enabled: boolean } | { siteName: string }) {
  const response = await fetch("/api/admin/settings", {
    method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input),
  });
  const payload = await response.json() as {
    registrations?: { enabled: boolean }; siteName?: string; error?: { message: string };
  };
  if (!response.ok) throw new Error(payload.error?.message ?? "Ayar kaydedilemedi.");
  return payload;
}

export function SiteSettingsManager({ initialEnabled, initialName }: { initialEnabled: boolean; initialName: string }) {
  ...
  async function save(input: { enabled: boolean } | { siteName: string }) {
